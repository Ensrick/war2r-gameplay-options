...
constexpr int kMaxHp = 9999;  // HP is signed 16-bit in places and the UI hides numbers from 10000 up

// Huge neutral objects whose "health" is not health: never scaled.
bool IsExcludedFromHealth(int type) { return type == kTypeGoldMine || type == 0x65 /* dark portal */ || type == 0x66 /* runestone */; }

int Scale(int value, double factor) { return static_cast<int>(value * factor + 0.5); }

void ScaleHealth() {
    const Config& c = config::g;
    if (c.hpUnits == 1.0 && c.hpHeroes == 1.0 && c.hpBuildings == 1.0) return;
    uint16_t* hp = At<uint16_t>(kRvaMaxHpByType);
    for (int t = 0; t < kTypeCount; ++t) {
        if (!hp[t] || IsExcludedFromHealth(t)) continue;
        const double k = c.isHero[t] ? c.hpHeroes : (t < units::kFirstBuilding ? c.hpUnits : c.hpBuildings);
...
void ScaleUnitCosts() {
    ...
    for (int t = 0; t < units::kFirstBuilding; ++t)
        for (uint8_t* table : {gold, lumber}) {
            ...
            table[t] = static_cast<uint8_t>(scaled < 1 ? 1 : (scaled > 255 ? 255 : scaled));
        }
}
...
void ScaleUpgradeCosts(const uint8_t* indices, int count, double k) {
    ...
    for (uint32_t rva : {kRvaUpgradeGold, kRvaUpgradeLumber, kRvaUpgradeOil}) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            table[indices[i]] = static_cast<uint16_t>(scaled > 65535 ? 65535 : scaled);
        }
    }
}
...
void OnNewMapTablesLoaded() {
    ...
    AddSight();
    logx::Write("map load: health x%.2f units / x%.2f heroes / x%.2f buildings, unit cost x%.2f, ranged upgrades x%.2f, "
                "siege upgrades x%.2f, sight bonuses applied",
                config::g.hpUnits, config::g.hpHeroes, config::g.hpBuildings, config::g.costUnits,
                config::g.costRangedUpgrades, config::g.costSiegeUpgrades);
}
