...
Steps (each one only when the file still needs it):
  ...
  1.16.0 adds the empty [weapon_types] / [armor_types] sections with two commented examples after [upgrades]
         (the example text is the 1.16.1 one)
  ...
  1.12.0 adds [auto_production] save_up_seconds = 60 after filler_min
...
def insert_after_key(lines, tree, section, after_key, block, notes):
    """Puts the key lines of `block` right under `after_key` in [section] (at the section's end when it is not there)."""
    have = tree.get(section, {})
    key = [l for l in block if not l.startswith('#')][0].split('=')[0].strip()
    if not isinstance(have, dict) or key in have:
        return
    s = span(lines, f'[{section}]')
    if s is None:
        return
    for i in range(s[0], s[1]):
        if '=' in lines[i] and lines[i].split('=')[0].strip() == after_key:
            lines[i + 1:i + 1] = block
...
def farms2_1280(lines, tree, notes):
    farms = tree.get('farms')
    if not isinstance(farms, dict) or 'mine_clearance' in farms:
        return
    # insert_after_key places the key lines of the block under free_percent; both keys go in together
    for i, line in enumerate(lines):
        if '=' in line and line.split('=')[0].strip() == 'free_percent':
            lines[i + 1:i + 1] = FARMS2_1280
            notes.append('[farms] gained mine_clearance, workers')
            return
...
def fix_ai_1260(lines, tree, notes):
    if 'general' in tree:
        insert_after_key(lines, tree, 'general', 'log_ai', FIX_AI_1260, notes)


def scouts_1250(lines, tree, notes):
    if 'scouts' in tree:
        return
    for i, line in enumerate(lines):
        if line.strip() == '[workers]':
            lines[i:i] = SCOUTS_1250 + ['']
            notes.append('added [scouts] (switched off)')
            return
    lines += [''] + SCOUTS_1250
    notes.append('added [scouts] at the end (switched off)')


def hold_1240(lines, tree, notes):
    if 'priority' in tree:
        insert_after_key(lines, tree, 'priority', 'save_mana', HOLD_1240, notes)
...
def resume_1180(lines, tree, notes):
    if 'autocast' in tree:
        insert_after_key(lines, tree, 'autocast', 'cast_while_attacking', RESUME_1180, notes)


def cooldown_1170(lines, tree, notes):
    if 'heal' in tree:
        append_missing(lines, tree, 'heal', COOLDOWN_1170, notes)


def types_1160(lines, tree, notes):
    """A config written before 1.16.0: the damage type sections go after [upgrades]. They hold no values, so nothing
    changes; they are there so the player finds them."""
    if any(k in tree for k in ('weapon_types', 'armor_types', 'damage_bonus')):
        return
    if any(l.strip() == '[weapon_types]' for l in lines):
        return
    s = span(lines, '[upgrades]')
    if s is None:
        lines += [''] + TYPES_1160
    else:
        end = s[1]
        ...
        lines[end:end] = [''] + TYPES_1160
    notes.append('added the empty [weapon_types] / [armor_types] sections (example commented out)')
...
def plenty_1140(lines, tree, notes, had_block):
    """plenty_units goes under filler_min. An existing auto-production setup keeps the old built-in 5."""
    prod = tree.get('auto_production')
    if not isinstance(prod, dict) or 'plenty_units' in prod:
        return
    block = list(PLENTY_1140)
    if had_block:
        block[-1] = 'plenty_units = 5            # the number the mod used before 1.14.0; new configs ship 10'
    insert_after_key(lines, tree, 'auto_production', 'filler_min', block, notes)


def clearance_1130(lines, tree, notes):
    if 'autocast' in tree:
        insert_after_key(lines, tree, 'autocast', 'area_building_value', CLEARANCE_1130, notes)


def save_up_1120(lines, tree, notes):
    if 'auto_production' in tree:
        insert_after_key(lines, tree, 'auto_production', 'filler_min', SAVE_UP_1120, notes)
...
def main():
    ...
    # The steps below add keys to [auto_production], which the step above may only just have created: they need the
    ...
    allowed = {('oil_platforms', 'unlimited'), ('oil_platforms', 'amount'), ('gold_mines', 'amount'), ('food', 'hall_food'),
               ...
               ('autocast', l.split('=')[0].strip()) for l in AUTOCAST_NEW if not l.startswith('#')} | {('autocast', 'area_building_value'), ('general', 'log_ai'), ('auto_production', 'save_up_seconds'), ('upgrades', 'missile_damage'), ('upgrades', 'melee_damage'), ('upgrades', 'shields'), ('upgrades', 'ship_damage'), ('upgrades', 'ship_armor'), ('upgrades', 'siege_damage'), ('auto_production', 'plenty_units'), ('autocast', 'area_friendly_clearance'), ('heal', 'cooldown_seconds'), ('heal', 'urgent_below_percent'), ('autocast', 'resume_orders'), ('priority', 'hold_for_blocked_area'), ('scouts', 'enabled'), ('general', 'fix_ai_after_load'), ('farms', 'auto_build'), ('farms', 'free_min'), ('farms', 'free_percent'), ('farms', 'mine_clearance'), ('farms', 'workers'), ('scouts', 'units'), ('scouts', 'toggle_key'), ('scouts', 'idle_seconds'), ('heal', 'cooldown_for_computer')} | {('priority', k) for k in ('paladin', 'mage', 'ogre_mage', 'death_knight', 'save_mana')} | {
