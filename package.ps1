...
if ($cmakeLists -notmatch 'project\(war2r_autocast VERSION ([0-9.]+)') { throw 'version not found in CMakeLists.txt' }
$version = $Matches[1]
if ($cmakeLists -match 'set\(AUTOCAST_PRERELEASE "([^"]*)"\)') { $version += $Matches[1] }
